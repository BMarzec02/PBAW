...
class PersonEditCtrl {
    ...
    public function validateSave() {
        ...
        $this->form->weight = $v->validateFromPost('weight', [
            'trim' => true,
            'required' => true,
            'required_message' => 'Podaj wagę przesyłki',
            'int' => true,
            'max_length' => 2,
            'validator_message' => 'Wagą powinna być w granicach 1-99kg'
        ]);
        $date = $v->validateFromPost('date', [
            'trim' => true,
            'required' => true,
            'required_message' => "Wprowadź datę nadania przesyłki",
            'date_format' => 'Y-m-d',
        ...
        $this->form->size  = $v->validateFromPost('size', [
            ...
            'required' => true,
            'max_length' => 1,
            'required_message' => 'Wprowadź rozmiar przesyłki',
            'validator_message' => "Niepoprawny rozmiar, dostępne rozmiary s/m/l"
        ]);
        $this->form->name  = $v->validateFromPost('name', [
            'trim' => true,
            'required' => true,
            'required_message' => 'Wprowadź imie',
            'validator_message' => ""
        ]);
        ...
        $this->form->address  = $v->validateFromPost('address', [
            'trim' => true,
            'required' => true,
            'required_message' => 'Wprowadź adres',
            'validator_message' => ""
        ]);
        $this->form->phone_number  = $v->validateFromPost('phone_number', [
            'trim' => true,
            'required' => true,
            'required_message' => 'Wprowadź numer tel',
            'validator_message' => ""
        ]);
    ...
    public function action_personEdit() {
        ...
        if ($this->validateEdit()) {
            try {
                ...
                $record = App::getDB()->get("parcel", "*", [
                    "id_parcel" => $this->form->id
                ]);
                // 2.1 jeśli osoba istnieje to wpisz dane do obiektu formularza
                $this->form->id = $record['id_parcel'];
        ...
        if ($this->validateSave()) {
            ...
            try {
                ...
                if ($this->form->id == '') {
                    ...
                    $count = App::getDB()->count("parcel");
                    if ($count <= 20) {
                        App::getDB()->insert("parcel", [
                            "name" => $this->form->name,
